...
library('vcfR')
library('dplyr')
vcf <- read.vcfR("StherPMEI.vcf", verbose = FALSE ) 
...
**6. Create maps for allele frequency, one gene per output file, where allele frequencies at each location are represented by pie charts:**
```{r, eval=F}
library(mapplots)
geneids <- unique(str_replace(colnames(matx), "_[0-9]+$", ""))
